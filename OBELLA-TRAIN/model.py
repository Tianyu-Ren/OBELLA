...
class AnswerEvaluationModel(nn.Module):
    def __init__(self, encoder_state, attention_func, pooling, num_class=2):
        super().__init__()
        self.encoder = AlbertModel.from_pretrained(encoder_state)
        # Get the model_dim and intermediate_dim
    ...
    def forward(self, reference_indicator, candidate_indicator, **inputs):
        # Encoding -> (batch_size, sequence_length, model_dim)
        x = self.encoder(**inputs).last_hidden_state
        # select span
        # x_reference -> (batch_size, n, model_dim); attention_mask -> (batch_size, n)
        x_reference, reference_mask = get_span_representation(x, reference_indicator)
        reference_mask = torch.repeat_interleave(reference_mask, self.args.num_attention_heads, 0)
        # x_candidate -> (batch_size, t, model_dim)
        x_candidate, candidate_mask = get_span_representation(x, candidate_indicator)
...
def get_tensor_inputs(tokenizer, question: list, reference: list, candidate: list, max_length):
    ...
    max_length = tensors.input_ids.shape[1]
...
def text_concat(text: (list, tuple), concat_token):
    concat_text = text[0]  # Expected to be questions list
    for i in text[1:]:
        concat_text = list(map(lambda x, y: x + concat_token + concat_token + y, concat_text, i))
    return concat_text


def model_evaluate(model, tokenizer, valid_iter, max_length, device):
    if next(model.parameters()).device != device:
        model = model.to(device)
    model.eval()
    true_false = []
    prediction = []
    for x in tqdm(valid_iter):
        ...
        true_false += (pred_labels == y_true).tolist()
    accuracy = sum(true_false) / len(true_false)
    return prediction, accuracy


def model_prediction(question: (list, tuple), reference, candidate, model, tokenizer, max_length, device):
    inputs, reference_span, candidate_span = get_tensor_inputs(tokenizer,
                                                               question, reference, candidate, max_length)
    inputs = {k: v.to(device) for k, v in inputs.items()}
    y_hat = model(reference_span, candidate_span, **inputs)
    return y_hat
